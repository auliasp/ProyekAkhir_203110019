...
class ListDrakorAdapter(val listDrakor: ArrayList<Drakor>) : RecyclerView.Adapter<ListDrakorAdapter.ListViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ListViewHolder, position: Int) {
        val drakor = listDrakor [position]
        Glide.with(holder.itemView.context)
            .load(drakor.photo)
            .apply(RequestOptions().override(55, 55))
            .into(holder.imgPhoto)
        holder.tvName.text = drakor.name
        holder.tvDetail.text = drakor.detail
